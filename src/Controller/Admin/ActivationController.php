    ...
    public function index()
    {
        if ($this->request->is('post')) {
            ...
            if (isset($result['item']['id']) && $result['item']['id'] == 16887109) {
                ...
                $personal_token = $Options->find()->where(['name' => 'zazapc'])->first();
                $personal_token->value = trim($this->request->data['zazapc']);
                $Options->save($personal_token);

                $purchase_code = $Options->find()->where(['name' => 'zazapc'])->first();
                $purchase_code->value = trim($this->request->data['zazapc']);
